...
从官方下载源码编译安装，安装后终端输入protoc --version报少.so的错时，在/etc/ld.so.conf.d/下创建bprotobuf.conf文件，写上/usr/local/lib，然后终端输入sudo ldconfig刷新.so动态库加载

4.aouth.h文件缺少问题

apt-get install libaouth-dev
